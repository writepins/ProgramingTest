...
public class TreeImplementation {
	...
	public static boolean isSubTree(Tree tree1, Tree tree2){
		...
		//else go to the tree1 left subtree or right subtree
	...
	public static boolean isBST(Tree tree, int small, int large){
		if (tree == null) return true;

		if (tree.value > small && tree.value < large){
			return isBST(tree.left, small, tree.value) && isBST(tree.right, tree.value, large);
		}else
			return false;
	}
	...
	//BFS application
	//Binary tree and a target Sum
	public static boolean hasPathSumIterative(Tree root, int sum) {
	...
	//print all the path
	public static void printTreePaths(Tree tree, int path[], int pathLen) { 
		if (tree == null) return;

		// append this node to the path array 
		path[pathLen] = tree.value; 
		pathLen++;
		...
		if (tree.left == null && tree.right == null) { 
			...
			printTreePaths(tree.left, path, pathLen); 
			printTreePaths(tree.right, path, pathLen); 
		} 
	}

	//print all the path given the sum
	public static void printTreePathsWithGivenSum(Tree tree, int sum, int path[], int pathLen) { 
		  if (tree == null) return;
		  ...
		  // it's a leaf, so print the path that led to here 
		  if (tree.left == null && tree.right == null && subsum == 0) { 
		    printArray(path, pathLen); 
		  } 
		  else { 
		  // otherwise try both subtrees 
